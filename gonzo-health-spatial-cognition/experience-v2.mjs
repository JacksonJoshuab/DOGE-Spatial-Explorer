const VERSION='gonzo-health-experience/2.3.5';
...
function compactText(value){return String(value||'').toLowerCase().replace(/[^a-z]+/g,'')}
function colorDescriptor(value){
  const compact=compactText(value);
  for(const [color,key] of Object.entries(responseKeys)){
    if(compact===color||compact===`${color}${key}`||compact===`${key}${color}`)return{color,key,label:color[0].toUpperCase()+color.slice(1)}
  }
  return null
}
function colorsIn(value){const compact=compactText(value);return Object.keys(responseKeys).filter(color=>compact.includes(color))}
...
function emitColorKey(descriptor){
  const init={key:descriptor.key,code:`Key${descriptor.key.toUpperCase()}`,bubbles:true,cancelable:true};
  document.dispatchEvent(new KeyboardEvent('keydown',init));
  document.dispatchEvent(new KeyboardEvent('keyup',init))
}
function markResponseTargets(examRoot){
  const rootRect=examRoot.getBoundingClientRect();
  const lowerBoundary=rootRect.top+rootRect.height*.55;
  const byColor=new Map();
  const leaves=$$('*',examRoot).filter(element=>element.children.length<=3&&colorDescriptor(element.innerText||element.textContent));
  for(const leaf of leaves){
    const descriptor=colorDescriptor(leaf.innerText||leaf.textContent);if(!descriptor)continue;
    const leafRect=leaf.getBoundingClientRect();
    if(!leafRect.width||!leafRect.height||leafRect.top<lowerBoundary)continue;
    let node=leaf;let target=null;
    for(let depth=0;node&&node!==examRoot&&depth<7;depth++,node=node.parentElement){
      const rect=node.getBoundingClientRect();
      const uniqueColors=colorsIn(node.innerText||node.textContent);
      if(uniqueColors.length!==1||uniqueColors[0]!==descriptor.color)continue;
      if(rect.top<lowerBoundary||rect.width<44||rect.height<40||rect.width>240||rect.height>140)continue;
      target=node;
      if(node.matches('button,[role="button"],[tabindex],[data-response],[data-color],label'))break
    }
    if(!target)continue;
    const area=target.getBoundingClientRect().width*target.getBoundingClientRect().height;
    const prior=byColor.get(descriptor.color);
    if(!prior||area<prior.area)byColor.set(descriptor.color,{target,descriptor,area})
  }
  for(const {target,descriptor} of byColor.values()){
    ...
    if(!target.matches('button,a,input,select,textarea,[role="button"]')){
      ...
      if(!target.dataset.ghKeyboardBound){
        ...
        target.addEventListener('keydown',event=>{
          if(event.key==='Enter'||event.key===' '){event.preventDefault();emitColorKey(descriptor)}
        })
      }
    }
  }
  examRoot.dataset.ghResponseCount=String(byColor.size)
}
function normalizeExamControls(examRoot){
  ...
  markResponseTargets(examRoot);const progress=$('[role="progressbar"],.progress');if(progress&&!progress.getAttribute('aria-label'))progress.setAttribute('aria-label','Assessment progress')
}
