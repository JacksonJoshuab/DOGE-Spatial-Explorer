...
function markResponseTargets(root){
  const rootRect=root.getBoundingClientRect();
  const bottomThreshold=rootRect.top+rootRect.height*.55;
  const candidates=$$('*',root).filter(element=>element.children.length<=4&&responseHotkeys.has(normalizedWords(element.textContent)));
  const marked=new Set();
  for(const leaf of candidates){
    const label=responseHotkeys.get(normalizedWords(leaf.textContent));
    const leafRect=leaf.getBoundingClientRect();
    if(!leafRect.width||!leafRect.height||leafRect.top<bottomThreshold)continue;
    let node=leaf;
    let best=null;
    for(let depth=0;node&&node!==root&&depth<5;depth++,node=node.parentElement){
      const rect=node.getBoundingClientRect();
      if(rect.top>=bottomThreshold&&rect.width>=44&&rect.height>=40&&rect.width<=Math.max(230,rootRect.width*.5))best=node;
    ...
    marked.add(target);
    target.dataset.ghResponse='';
    target.setAttribute('aria-label',`Respond ${label}`);
    if(!target.matches('button,a,input,select,textarea,[role="button"]')){
      target.setAttribute('role','button');
      if(!target.hasAttribute('tabindex'))target.tabIndex=0;
      if(!target.dataset.ghKeyboardBound){
        target.dataset.ghKeyboardBound='true';
...
function normalizeExamControls(root){
  const candidates=$$('button,[role="button"],[tabindex]');
  for(const element of candidates){
    const name=accessibleName(element);
    if(/pause|Ⅱ|⏸/i.test(name)){element.dataset.action='pause';if(!element.getAttribute('aria-label'))element.setAttribute('aria-label','Pause assessment')}
    else if(/stop session|end session|exit assessment|close assessment|×|✕|✖/i.test(name)){element.dataset.action='stop';if(!element.getAttribute('aria-label'))element.setAttribute('aria-label','Stop assessment')}
  }
  markResponseTargets(root);
  const progress=$('[role="progressbar"],.progress');if(progress&&!progress.getAttribute('aria-label'))progress.setAttribute('aria-label','Assessment progress')
}
...
function enhanceExam(root){root.dataset.ghExamRoot=VERSION;document.body.dataset.ghExam='active';document.body.dataset.ghView='assessment';normalizeExamControls(root);ensureSessionGuide()}
...
function enhanceResults(root){
  document.body.dataset.ghView='results';
  if(root.dataset.ghReport===VERSION)return;
  root.dataset.ghReport=VERSION;root.dataset.ghReportRoot=VERSION;
  const section=document.createElement('section');section.className='gh-report-explainer';section.innerHTML='<div><span class="eyebrow">How to read this</span><h2>One session is a snapshot, not a diagnosis.</h2><p>Start with session validity and accuracy, then review response speed and consistency. Trends become more useful after repeated comparable sessions.</p></div><div class="gh-report-legend"><span><i class="good"></i>Stable strength</span><span><i class="watch"></i>Worth watching</span><span><i class="caution"></i>Use caution</span></div>';
  const score=root.querySelector('.score,.score-ring,.hero-score,[data-score]');if(score)score.insertAdjacentElement('afterend',section);else root.prepend(section);
  const next=document.createElement('section');next.className='gh-next-actions';next.innerHTML='<h3>Recommended next steps</h3><div><button type="button" data-gh-repeat>Repeat comparable check</button><button type="button" data-gh-history>Review trend</button><button type="button" data-gh-export>Export evidence</button></div><small>Useful goals include a metric, baseline, target, timeframe, and guardrail.</small>';root.append(next);
  next.querySelector('[data-gh-repeat]')?.addEventListener('click',()=>location.href='./?battery=daily');
  next.querySelector('[data-gh-history]')?.addEventListener('click',()=>app.querySelector('[data-nav="history"]')?.click());
  next.querySelector('[data-gh-export]')?.addEventListener('click',()=>app.querySelector('[data-action="export"],button[title*="Export" i],button[aria-label*="Export" i]')?.click())
}
...
function enhance(){
  ...
  addSkip();normalizeLobbyActions();annotateBattery();addReadiness();addReportNav();
  const exam=findExamRoot();
  if(exam)enhanceExam(exam);
  else{
    removeSessionGuide();
    const results=findResultsRoot();
    if(results)enhanceResults(results);else document.body.dataset.ghView='lobby'
  }
  document.documentElement.dataset.ghExperience=VERSION
}
let scheduled=false;new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;enhance()})}).observe(app,{subtree:true,childList:true});keyboard();enhance();window.addEventListener('pageshow',enhance);window.__GONZO_HEALTH_EXPERIENCE__=Object.freeze({version:VERSION,examExpanded:true,reportingExpanded:true,stateDetection:'explicit',responseSemantics:'geometry-hotkey'});